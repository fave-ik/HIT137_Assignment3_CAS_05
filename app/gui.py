...
class App(ttk.Frame):
    ...
    def _build_layout(self):
        ...
        ttk.Button(run_row, text="Run Model 2 (Image)", command=self._run_image).pack(
            side="left", padx=8
        )
        ...
        left_info = ttk.Frame(info)
        left_info.pack(side="left", fill="x", expand=True, padx=(0, 6))
        right_info = ttk.Frame(info)
        right_info.pack(side="left", fill="x", expand=True, padx=(6, 0))

        ttk.Label(left_info, text="Selected Model Info", font=("", 10, "bold")).pack(anchor="w")
        self.info_name = ttk.Label(left_info, text="Model: -")
